...
medics-own [ skillset W S joined ]
...
to find-co
  foreach ([who] of tasks) [ ?1 -> ask medics [ find-coalition who ?1 ] ]
end

to setup-medics
  output-show "skillset:"
  ask medics [
    set skillset build-list-randomly skill_amount 1
    set W []
    set S []
    set label who
    set joined []
...
to find-coalition [ medic-i task_i ]
  let L_i 0
  let Max_i 0
  let w_max 0
  let S_max 0
...
to-report max-coalition [ c_lst task_i ]
  ...
  foreach c_lst [ ?1 ->
    ...
    foreach ?1 [ ??1 ->
      set index 0
      set current_skills [skillset] of (medic ??1)
      repeat skill_amount [
