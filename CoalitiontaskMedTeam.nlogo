...
medics-own [ my-skills W S joined ]
works-own [ requirements coalition-in-use ]

globals [ L ]

to setup
  __clear-all-and-reset-ticks
  create-medics medics_available
  create-works patients_amount
  setup-medics
  setup-works
  setup-L
end
...
to find-co
  foreach ([who] of works) [ ?1 -> ask medics [ find-coalition who ?1 ] ]
end

to setup-medics
  output-show "my-skills:"
  ask medics [
    set my-skills build-list-randomly skill_amount 1
    set W []
    set S []
    set label who
    set joined []
    ...
    set size 2
    set xcor -13
    set ycor (30 / medics_available) * ceiling(who / 2) * (-1)^(who)
    output-show my-skills ]
end
...
to setup-works
  ...
  ask works [
    ...
    set requirements build-list-randomly skill_amount skill_requirements
    set shape "person"
    set size 3
    set color (color - 20)
    set xcor 13
    set ycor (30 / patients_amount) * ceiling((who - medics_available) / 2) * (-1)^(who)
    output-show requirements ]
end

to setup-L
  set L []
  ask medics [ set L lput (list who) L ]
  repeat medics_available [
    ask medics [
      without-interruption [
        foreach L [ ?1 ->
          if( (not member? who ?1) ) [ set L lput (sort(lput who ?1)) L ]
...
to find-coalition [ medic-i work_i ]
  let L_i 0
  let Max_i 0
  let w_max 0
  let S_max 0

  set L_i coalitions medic-i

  while [ not( empty? L_i ) ] [
    set W []
    set S []
    set Max_i  max-coalition L_i work_i
    broadcast Max_i
    set w_max item (find-index-of-max [W] of (medic medic-i)) W
    set S_max item (position w_max W) S
    foreach S_max [ ?1 ->
      if( ?1 = medic-i ) [
        set joined S_max
        create-link-with (work work_i) [ set color (((position S_max L_i) mod 7) * 10 + 35) ]
        set heading towards work work_i
        ask work work_i [
    ...
    foreach S_max [ ?1 ->
      ...
      foreach L_i [ ??1 ->
        if( member? i ??1 ) [
          set L_i remove ??1 L_i ] ] ] ]
end

to-report coalitions [ i ]
  let lst 0
  set lst []
  foreach L [ ?1 -> if( member? i ?1 ) [ set lst lput ?1 lst ] ]
  report lst
end

to-report max-coalition [ c_lst work_i ]
  let maximum-value 0
  let maximum-col 0
  let sum_vector 0
  ...
  foreach c_lst [ ?1 ->
    ...
    foreach ?1 [ ??1 ->
      set index 0
      set current_skills [my-skills] of (medic ??1)
      repeat skill_amount [
    ...
    repeat skill_amount [
      set sum_vector replace-item index sum_vector (-1 * abs( precision ( (item index [requirements] of (work work_i)) - (item index sum_vector) ) 2 ))
      set index ( index + 1 ) ]
    if ( ((sum sum_vector) / (length ?1)) >= maximum-value ) [
      set maximum-value precision ((sum sum_vector) / (length ?1)) 2
      set maximum-col ?1 ] ]
